...
5. `mistake_notes`（`question_id` → `questions`）
6. `attempts`（`question_id`, `mistake_note_id`）
...
| 4 | `GET/PUT /mistake-notes`, `GET /mistake-notes/today` | questions に依存 |
| 5 | `POST /questions/{id}/attempts` | mistake_notes の更新を含む |
| 6 | `PUT /mistake-notes/{id}/mastered` | mistake_notes に依存 |
